<section id="formularz">
    <div class="podsekcja" id="C">
        ...
        <form action="php/admin/formularze/main_formularz.php" method="POST">
            <input type="hidden" name="formularz" value="pracownik">
            ...
            <!-- poziom_uprawnien	enum('administrator', 'bibliotekarz', 'uzytkownik') -->
            <label for="poziom_uprawnien">Poziom uprawnień:</label>
            <select id="poziom_uprawnien" name="poziom_uprawnien" required>
                <option value="administrator" <?php echo (get_form_value('poziom_uprawnien') == 'administrator') ? 'selected' : ''; ?>>Administrator</option>
                <option value="bibliotekarz" <?php echo (get_form_value('poziom_uprawnien') == 'bibliotekarz') ? 'selected' : ''; ?>>Bibliotekarz</option>
                <option value="uzytkownik" <?php echo (get_form_value('poziom_uprawnien') == 'uzytkownik') ? 'selected' : ''; ?>>Użytkownik</option>
            </select>
    ...
    <div class="podsekcja" id="U">
        ...
        <form action="php/admin/formularze/main_formularz.php" method="POST">
            ...
            <label for="new_poziom_uprawnien">Nowy poziom uprawnień:</label>
            <select id="new_poziom_uprawnien" name="new_poziom_uprawnien" required>
                <option value="administrator" <?php echo (get_form_value('new_poziom_uprawnien') == 'administrator') ? 'selected' : ''; ?>>Administrator</option>
                <option value="bibliotekarz" <?php echo (get_form_value('new_poziom_uprawnien') == 'bibliotekarz') ? 'selected' : ''; ?>>Bibliotekarz</option>
                <option value="uzytkownik" <?php echo (get_form_value('new_poziom_uprawnien') == 'uzytkownik') ? 'selected' : ''; ?>>Użytkownik</option>
            </select>
    ...
    <script>
        ...
        document.getElementById('editID').addEventListener('input', function() {
            ...
            if (id) { // Jeśli ID nie jest puste
                const queryType = 'pracownik_edit'; // Typ zapytania
                ...
                fetch(`php/admin/formularze/fetch_edit_data.php?editID=${id}&queryType=${queryType}`)
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            document.getElementById('new_poziom_uprawnien').value = data.data.poziom_uprawnien;
                            ...
                            document.getElementById('new_poziom_uprawnien').value = '';
